		...
		$().ready(function() {
				...
				$shippingMethodItem.each(function() {
					...
					if ($.inArray(paymentMethodId, paymentMethodIds[shippingMethodId]) >= 0) {
						...
						if ($element.hasClass("active")) {
							...
							$currentShippingMethod.find("span.name").text("${message("shop.common.choose")}");
						}
					}
				});
			});
				...
				$paymentMethodItem.each(function() {
					...
					if ($.inArray(paymentMethodId, paymentMethodIds[shippingMethodId]) >= 0) {
						...
						if ($element.hasClass("active")) {
							$element.removeClass("active");
							$paymentMethodId.val("");
							$currentPaymentMethod.find("span.name").text("${message("shop.common.choose")}");
						}
					}
				});
			});
			...
			$submit.click(function() {
				...
				$.ajax({
					...
					success: function(data) {
						location.href = amountPayable > 0 ? "payment?sn=" + data.sn : "${base}/member/order/view?orderSn=" + data.sn;
					},
					complete: function() {
					...
					<div class="list-group list-group-flat">
						...
						[#if order.isDelivery]
							<div id="currentShippingMethod" class="list-group-item text-right" data-toggle="page" data-target="#shippingMethodPage">
								...
								<span class="name gray-darker">${(defaultShippingMethod.name)!message("shop.common.choose")}</span>
								<span class="glyphicon glyphicon-menu-right"></span>
							</div>
						[/#if]
						[#if order.type == "general"]
								...
								<div class="media">
									<div class="media-left media-middle">
										<a href="${base}${orderItem.sku.path}">
											<img class="media-object" src="${orderItem.thumbnail!setting.defaultThumbnailProductImage}" alt="${orderItem.name}">
										</a>
									</div>
									<div class="media-body media-middle">
										<h4 class="media-heading">
					...
					<div class="list-group list-group-flat">
						[#if order.type == "general"]
							...
							<div class="list-group-item small">
								...
								<strong class="pull-right red" data-name="promotionDiscount" data-type="currency">${currency(order.promotionDiscount, true, true)}</strong>
							</div>
							<div class="list-group-item small">
								${message("Order.couponDiscount")}
								<strong class="pull-right red" data-name="couponDiscount" data-type="currency">${currency(order.couponDiscount, true, true)}</strong>
							</div>
							[#if setting.isInvoiceEnabled && setting.isTaxPriceEnabled]
								<div class="list-group-item small">
									${message("Order.tax")}
									<strong class="pull-right red" data-name="tax" data-type="currency">${currency(order.tax, true, true)}</strong>
								</div>
							[/#if]
						[#elseif order.type == "exchange"]
							<div class="list-group-item small">
								${message("shop.order.currentPoint")}
								<strong class="pull-right red">${currentUser.point}</strong>
							</div>
						[/#if]
						[#if order.isDelivery]
							<div class="list-group-item small">
								${message("Order.freight")}
								<strong class="pull-right" data-name="freight" data-type="currency">${currency(order.freight, true, true)}</strong>
							</div>
						[/#if]
					</div>
				</div>
					...
					<div class="row">
						<div class="col-xs-8 text-center">
							[#if order.type == "general"]
								...
								<strong class="red" data-name="amount" data-type="currency">${currency(order.amount, true, true)}</strong>
							[#elseif order.type == "exchange"]
