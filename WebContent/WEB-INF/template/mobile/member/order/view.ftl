		...
		$().ready(function() {
			...
			$payment.click(function() {
				$.ajax({
					...
					dataType: "json",
					success: function() {
						location.href = "${base}/order/payment?orderSn=${order.sn}";
					}
				});
				return false;
			});
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button class="btn btn-sm btn-default" type="button" data-dismiss="modal">${message("member.common.close")}</button>
				</div>
			</div>
		</div>
	</div>
